...
func (cu *ConfUpdate) SetConfs(c *confs.Confs) *ConfUpdate {
	cu.mutation.SetConfs(c)
	return cu
}
...
func (cu *ConfUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if value, ok := cu.mutation.Confs(); ok {
...
// SetConfs sets the confs field.
func (cuo *ConfUpdateOne) SetConfs(c *confs.Confs) *ConfUpdateOne {
	cuo.mutation.SetConfs(c)
	return cuo
}
...
func (cuo *ConfUpdateOne) sqlSave(ctx context.Context) (_node *Conf, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "ID", err: fmt.Errorf("missing Conf.ID for update")}
	}
	_spec.Node.ID.Value = id
	if value, ok := cuo.mutation.Confs(); ok {
		_spec.Fields.Set = append(_spec.Fields.Set, &sqlgraph.FieldSpec{
			Type:   field.TypeJSON,
			Value:  value,
			Column: conf.FieldConfs,

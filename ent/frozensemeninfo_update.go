...
// Where appends a list predicates to the FrozenSemenInfoUpdate builder.
func (fsiu *FrozenSemenInfoUpdate) Where(ps ...predicate.FrozenSemenInfo) *FrozenSemenInfoUpdate {
	fsiu.mutation.Where(ps...)
	return fsiu
}
...
func (fsiu *FrozenSemenInfoUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if ps := fsiu.mutation.predicates; len(ps) > 0 {
...
func (fsiuo *FrozenSemenInfoUpdateOne) Select(field string, fields ...string) *FrozenSemenInfoUpdateOne {
	fsiuo.fields = append([]string{field}, fields...)
	return fsiuo
}

// Save executes the query and returns the updated FrozenSemenInfo entity.
func (fsiuo *FrozenSemenInfoUpdateOne) Save(ctx context.Context) (*FrozenSemenInfo, error) {
	...
	if len(fsiuo.hooks) == 0 {
		...
		for i := len(fsiuo.hooks) - 1; i >= 0; i-- {
			if fsiuo.hooks[i] == nil {
				return nil, fmt.Errorf("ent: uninitialized hook (forgotten import ent/runtime?)")
			}
			mut = fsiuo.hooks[i](mut)
		}
		v, err := mut.Mutate(ctx, fsiuo.mutation)
		if err != nil {
			return nil, err
		}
...
func (fsiuo *FrozenSemenInfoUpdateOne) sqlSave(ctx context.Context) (_node *FrozenSemenInfo, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent: missing "FrozenSemenInfo.id" for update`)}
	}
	_spec.Node.ID.Value = id
	if fields := fsiuo.fields; len(fields) > 0 {
		...
		for _, f := range fields {
			...
			if f != frozensemeninfo.FieldID {
				_spec.Node.Columns = append(_spec.Node.Columns, f)
			}
		}
	}
	if ps := fsiuo.mutation.predicates; len(ps) > 0 {
		_spec.Predicate = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}

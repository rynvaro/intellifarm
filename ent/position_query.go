...
func (pq *PositionQuery) FirstXID(ctx context.Context) int64 {
...
func (pq *PositionQuery) IDs(ctx context.Context) ([]int64, error) {
	var ids []int64
...
func (pq *PositionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if ps := pq.order; len(ps) > 0 {
		_spec.Order = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector, position.ValidColumn)
			}
		}
	}
	return _spec
...
func (pq *PositionQuery) sqlQuery() *sql.Selector {
	...
	for _, p := range pq.order {
		p(selector, position.ValidColumn)
	}
	if offset := pq.offset; offset != nil {
...
func (pgb *PositionGroupBy) sqlQuery() *sql.Selector {
	...
	for _, fn := range pgb.fns {
		columns = append(columns, fn(selector, position.ValidColumn))
	}

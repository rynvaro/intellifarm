...
// Where appends a list predicates to the ReproductionParametersUpdate builder.
...
func (rpuo *ReproductionParametersUpdateOne) Select(field string, fields ...string) *ReproductionParametersUpdateOne {
	rpuo.fields = append([]string{field}, fields...)
	return rpuo
}

// Save executes the query and returns the updated ReproductionParameters entity.
...
func (rpuo *ReproductionParametersUpdateOne) sqlSave(ctx context.Context) (_node *ReproductionParameters, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent: missing "ReproductionParameters.id" for update`)}
	}
	...
	if fields := rpuo.fields; len(fields) > 0 {
		...
		for _, f := range fields {
			...
			if f != reproductionparameters.FieldID {

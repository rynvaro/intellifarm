...
// Where appends a list predicates to the ShedSettingUpdate builder.
...
func (ssuo *ShedSettingUpdateOne) Select(field string, fields ...string) *ShedSettingUpdateOne {
	ssuo.fields = append([]string{field}, fields...)
	return ssuo
}

// Save executes the query and returns the updated ShedSetting entity.
...
func (ssuo *ShedSettingUpdateOne) sqlSave(ctx context.Context) (_node *ShedSetting, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent: missing "ShedSetting.id" for update`)}
	}

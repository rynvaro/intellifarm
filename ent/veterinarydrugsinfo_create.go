...
func (vdic *VeterinaryDrugsInfoCreate) Save(ctx context.Context) (*VeterinaryDrugsInfo, error) {
	...
	if len(vdic.hooks) == 0 {
		...
		if _, err := mut.Mutate(ctx, vdic.mutation); err != nil {
			return nil, err
		}
	}
	return node, err
...
func (vdic *VeterinaryDrugsInfoCreate) check() error {
	...
	if _, ok := vdic.mutation.StopAt(); !ok {
		return &ValidationError{Name: "stopAt", err: errors.New("ent: missing required field \"stopAt\"")}
	}
...
// VeterinaryDrugsInfoCreateBulk is the builder for creating a bulk of VeterinaryDrugsInfo entities.
		...
		func(i int, root context.Context) {
			...
			var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
				...
				if i < len(mutators)-1 {
					...
					if err = sqlgraph.BatchCreate(ctx, vdicb.driver, &sqlgraph.BatchCreateSpec{Nodes: specs}); err != nil {
						if cerr, ok := isSQLConstraintError(err); ok {
							err = cerr
						}
					}

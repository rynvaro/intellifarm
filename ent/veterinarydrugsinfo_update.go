...
// AddStopAt adds i to the "stopAt" field.
...
func (vdiu *VeterinaryDrugsInfoUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if ps := vdiu.mutation.predicates; len(ps) > 0 {
...
func (vdiuo *VeterinaryDrugsInfoUpdateOne) Select(field string, fields ...string) *VeterinaryDrugsInfoUpdateOne {
	vdiuo.fields = append([]string{field}, fields...)
	return vdiuo
}

// Save executes the query and returns the updated VeterinaryDrugsInfo entity.
func (vdiuo *VeterinaryDrugsInfoUpdateOne) Save(ctx context.Context) (*VeterinaryDrugsInfo, error) {
	...
	if len(vdiuo.hooks) == 0 {
		...
		for i := len(vdiuo.hooks) - 1; i >= 0; i-- {
			if vdiuo.hooks[i] == nil {
				return nil, fmt.Errorf("ent: uninitialized hook (forgotten import ent/runtime?)")
			}
			mut = vdiuo.hooks[i](mut)
		}
		v, err := mut.Mutate(ctx, vdiuo.mutation)
		if err != nil {
			return nil, err
		}
...
func (vdiuo *VeterinaryDrugsInfoUpdateOne) sqlSave(ctx context.Context) (_node *VeterinaryDrugsInfo, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent: missing "VeterinaryDrugsInfo.id" for update`)}
	}
	_spec.Node.ID.Value = id
	if fields := vdiuo.fields; len(fields) > 0 {
		...
		for _, f := range fields {
			...
			if f != veterinarydrugsinfo.FieldID {
				_spec.Node.Columns = append(_spec.Node.Columns, f)
			}
		}
	}
	if ps := vdiuo.mutation.predicates; len(ps) > 0 {
		_spec.Predicate = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
